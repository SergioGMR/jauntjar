...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:sidebar sticky stashable class="border-r border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group class="grid">
                <flux:navlist.item icon="trophy" :href="route('clasificados.listado')"
                    :current="request()->routeIs('clasificados.listado')" wire:navigate>{{ __('Clasificación') }}
                </flux:navlist.item>
            </flux:navlist.group>
        </flux:navlist>
                        ...
                        <div class="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
                            <span class="relative flex h-8 w-8 shrink-0 overflow-hidden rounded-lg">
                                <span
                                    ...
                                    {{ auth()->user()?->initials() ?? 'UK' }}
                                </span>
                            </span>

                            <div class="grid flex-1 text-left text-sm leading-tight">
                                <span class="truncate font-semibold">{{ auth()->user()?->name ?? 'Unknown' }}</span>
                                <span class="truncate text-xs">{{ auth()->user()?->email ?? 'Unknown' }}</span>
                            </div>
                        </div>
                    </div>
                </flux:menu.radio.group>
